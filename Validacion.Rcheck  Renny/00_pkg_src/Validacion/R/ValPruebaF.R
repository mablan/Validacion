...
function(x,y) {
##########################################
#Esta función calcula la prueba F que tiene como hipótesis nula que el
...
numreal<-length(y)
numsim<-length(x)
Famb<-matrix(nrow=numreal,ncol=numsim,dimnames=list(paste("Observación", 1:numreal), paste("Modelo", 1:numsim)))
ValP<-matrix(nrow=numreal,ncol=numsim,dimnames=list(paste("Observación", 1:numreal), paste("Modelo", 1:numsim)))
for (i in 1:length(x)){
for (j in 1:length(y)){
xxx<-x[i]
yyy<-y[j]
...
xx<-as.matrix(xx)
yy<-as.matrix(yy)
m1<-lm(yy~xx) # Se hace la regresión lineal con pendiente 1 e intercepto 0
